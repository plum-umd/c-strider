...
void serial_init(){
  ...
  ser = fopen("ser.txt", "wb"); //TODO generalize
...
int serial_ptr(void **in, typ type, void **out){
  ...
  char *symbol;
  if ((symbol = kitsune_lookup_addr_old(*in))) {
...
int deserial_ptr(void **in, typ type, void **out){
   ...
   if(present == (void*)SER_SYMBOL){
      ...
      *out = kitsune_lookup_key_new(symbol); //will write null if not found 
      free(symbol);
   }
   else if(present == NULL){ // null pointer
      *out = 0;
   }
   else if((tgt = transform_find_mapping(present))) {
         *out = tgt;
   } 
   else { //haven't seen it ; read the data
      if(cstrider_get_ptrtype(type) == TYPE_CHAR_NT){ //string
         deserial_char_str(&tgt); //getting sz/malloc happens here.
         *out = tgt;
         transform_add_mapping(present, tgt);
      } 
      else{
         typ next_type = cstrider_get_ptrtype(type);
         int to_alloc = cstrider_get_size(next_type);
         if(cstrider_is_array(next_type))
             to_alloc = cstrider_get_array_len(next_type); // malloc the array itself (just the array...if it's an array of poitners, pt'ed to gets malloced later) 
         tgt = malloc(to_alloc);
         transform_add_mapping(present, tgt);
         *out = tgt;
         cstrider_traverse(tgt, tgt, next_type);
      }
   }
   return 0;
}
...
void deserial_ptr_mapped(void **in, typ type, void **out){
      kitsune_log("should be null, and should have been not mapped. instead was: %d\n", in);
      assert(0);
}
